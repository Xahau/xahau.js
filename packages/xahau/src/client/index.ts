...
import { dropsToXah, hashes, isValidClassicAddress } from '../utils'
...
export interface ClientOptions extends ConnectionUserOptions {
  ...
  feeCushion?: number
  /**
   * Maximum transaction cost to allow, in decimal XAH. Must be a string-encoded
 ...
 *
 * @param command - The xahaud request command.
...
/**
 * Client for interacting with xahaud servers.
 *
 * @category Clients
 */
class Client extends EventEmitter<EventTypes> {
  /*
   * Underlying connection to xahaud.
   */
   ...
   * @category Fee
   */
  public readonly maxFeeXAH: string
  ...
  /**
   * Creates a new Client with a websocket connection to a xahaud server.
   *
  ...
  public constructor(server: string, options: ClientOptions = {}) {
    ...
    this.maxFeeXAH = options.maxFeeXAH ?? DEFAULT_MAX_FEE_XAH
   ...
   * general use. Instead, use xahaud's built-in pagination and make multiple
   * requests as needed.
   *
   * @category Network
   *
  ...
  >(request: T, collect?: string): Promise<U[]> {
    ...
    do {
      ...
      if (!(collectKey in singleResult)) {
        throw new XahlError(`${collectKey} not in result`)
      }
  ...
  /**
   * Tells the Client instance to connect to its xahaud server.
   *
   * @example
   *
  ...
  /**
   * Disconnects the XAHL client from the server and cancels all pending requests and subscriptions. Call when
  ...
  /**
   * Checks if the Client instance is connected to its xahaud server.
   *
   ...
   *     Amount: '10000000' // 10 XAH in drops (1/1,000,000th of an XAH)
   *   }
   *
   *   try {
   ...
   * @throws XahaudError if submit request fails.
   *
   * @example
   * ```ts
  ...
  /**
   * Retrieves the XAH balance of a given account address.
   *
   ...
   * ```
   *
   * @param address - The XAH address to retrieve the balance for.
   ...
   * @returns A promise that resolves with the XAH balance as a number.
   */
  public async getXrpBalance(
    ...
    return dropsToXah(response.result.account_data.Balance)
  }

  /**
   * Get XAH/non-XAH balances for an account.
   *
   * @category Abstraction
   *
   * @example
   ...
   * @returns An array of XAH/non-XAH balances for the given account.
   */
   ...
   * To send it XAH on ledger to make it a real account. If successful, this will return the new account balance in XAH
   ...
   *       faucetPath: '/accounts'
   *     })
   *     console.log(`Sent 10 XAH to wallet: ${address} from the given faucet. Resulting balance: ${balance} XAH`)
   *   } catch (error) {
   ...
   * ```
   *
   * @param wallet - An existing XAHL Wallet to fund. If undefined or null, a new Wallet will be created.
   ...
   * @returns A Wallet on the Testnet or Devnet that contains some amount of XAH,
   * and that wallet's balance in XAH.
  ...
  public async fundWallet(
    ...
    if (!this.isConnected()) {
      throw new XahaudError('Client not connected, cannot call faucet')
    }
