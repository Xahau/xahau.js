...
export async function calculateFeePerTransactionType(
  ...
  const netFeeDrops = xahToDrops(netFeeXRP)
  ...
  const maxFeeDrops = xahToDrops(client.maxFeeXRP)
  const totalFee =
    tx.TransactionType === 'AccountDelete'
      ? baseFee
 ...
 * @param client - The client object.
 * @param tx - The transaction object.
 * @returns A promise that resolves with void if there are no blockers, or rejects with an XahlError if there are blockers.
 */
