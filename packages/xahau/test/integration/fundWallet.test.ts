...
async function generate_faucet_wallet_and_fund_again(
  ...
  assert.equal(dropsToXah(afterSent.result.account_data.Balance), newBalance)
...
describe('fundWallet', function () {
  ...
  it(
    ...
    async function () {
      await generate_faucet_wallet_and_fund_again('wss://xahau-test.net:51233')
    },
    TIMEOUT,
  )
